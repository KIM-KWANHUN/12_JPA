...
public class BiTest {
    ...
    *   즉 주인을 설정하는 기준은 외래키를 가지고 있는 엔티티가 주인이 되어야 한다.
    *   따라서 우리가 쓰는 테이블 따지면 Menu 가 categoryCode 가지기 때문에 주인이다. */
    ...
    void slaveFindTest() {
        int categoryCode = 10;
        // 주인이 아닌 객체 조회 시 해당 엔티티를 먼저 조회 후, 필요 시 연관 된 엔티티 조회하는 쿼리실행
