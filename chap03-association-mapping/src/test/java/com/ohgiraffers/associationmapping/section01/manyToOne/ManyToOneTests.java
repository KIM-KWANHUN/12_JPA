...
public class ManyToOneTests {
    ...
    *   2. 객체 지향 쿼리(JPQL) 사용이 된다. */
    ...
    void manyToOneFind() {
        ...
        Menu menu = manyToOneService.find(menuCode);

        System.out.println("메뉴가 잘들어왔는지 확인용 = " + menu);

        Category category = menu.getCategory();

        System.out.println("카테고리가 잘들어왔는지 확인용 = " + category);

        Assertions.assertNotNull(category);
    }

    @DisplayName("N:1 관계 객체지향쿼리 이용 테스트")
    @Test
    void manyToOneJPQL() {

        int menuCode = 11;

        String categoryName = manyToOneService.findCategoryName(menuCode);
    ...
    *   카테고리 엔티티도 insert 시켜야 한다는 의미이다. */

    private static Stream<Arguments> getInfo(){
        return Stream.of(
                Arguments.of(
                      100, "돈까스덮밥", 30000, 123, "퓨전분식", "Y"
                )
        );
    }
    ...
    void manyToOneInsert(int menuCode, String menuName, int menuPrice,
        ...
        MenuDTO menu = new MenuDTO(
                ...
                new CategoryDTO(
                        categoryCode, categoryName, null),
                        orderableStatus
        );
