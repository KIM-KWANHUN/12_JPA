...
public class LifeCycleTest {
    ...
    @ParameterizedTest
    @DisplayName("비영속 테스트 (관리하지 않을때)")
    @ValueSource(ints = {1, 2})
    void testNotManaged(int menuCode) {

        Menu menu = lifeCycle.findMenuByMenuCode(menuCode);
        ...
        Assertions.assertNotEquals(menu, newMenu);
    }
   ...
   void testOtherManager(int menuCode) {
    ...
    /* comment.
    *   엔티티 매니저가 영속성 컨텍스트 엔티티 객체를 저장(persist)
    ...
    *   때문에 같은 객체(인스턴스) 를 반환하게 된다. */
    ...
    void testDetachEntity(int menuCode, int menuPrice) {
        ...
        Menu menu = manager.find(Menu.class, menuCode);

        transaction.begin();

        // detach : 특정 엔티티만 준영속 상태(관리하지 않는 상태)
        manager.detach(menu);
    ...
    void testDetachAndPersist(int menuCode, int menuPrice) {
        ...
        /* comment.
        *   파라미터로 넘어 온 menu 준영속 엔티티 객체의 식별자 값으로
    ...
    void testRemoveEntity(int menuCode) {
        ...
        /* comment.
        *   remove() 엔티티를 영속성 컨텍스트 및 데이터베이스에서 제거한다.
        *   단 트랜젝션을 제어하지 않으면 데이터베이스에 영구 반영되지는 않는다.
        *   트랜젝션을 커밋, 플러쉬 하는 순간 영속성 컨텍스트 에서 관리하는
        *   엔티티 객체를 DB 에 반영을 하게 된다. */
        ...
        // 같은 메뉴코드로 동일한 데이터 찾아가는지 확인
        Menu reMenu = manager.find(Menu.class, menuCode);

        // null 인지 테스트
        Assertions.assertNull(reMenu);
    }
}
